...
use Tests\Model\Answer as ChildAnswer;
use Tests\Model\AnswerQuery as ChildAnswerQuery;
use Tests\Model\Map\AnswerTableMap;
 ...
 *
 *
 * @method     ChildAnswerQuery orderByLabel($order = Criteria::ASC) Order by the label column
 * @method     ChildAnswerQuery orderByIdQuestion($order = Criteria::ASC) Order by the id_question column
 * @method     ChildAnswerQuery orderById($order = Criteria::ASC) Order by the id column
 *
 * @method     ChildAnswerQuery groupByLabel() Group by the label column
 * @method     ChildAnswerQuery groupByIdQuestion() Group by the id_question column
 ...
 *
 * @method     \Tests\Model\QuestionQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildAnswer findOneByLabel(string $label) Return the first ChildAnswer filtered by the label column
 * @method     ChildAnswer findOneByIdQuestion(int $id_question) Return the first ChildAnswer filtered by the id_question column
 ...
 * @method     ChildAnswer[]|ObjectCollection findByLabel(string $label) Return ChildAnswer objects filtered by the label column
 * @method     ChildAnswer[]|ObjectCollection findByIdQuestion(int $id_question) Return ChildAnswer objects filtered by the id_question column
...
abstract class AnswerQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Tests\Model\Base\AnswerQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'tests', $modelName = '\\Tests\\Model\\Answer', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT label, id_question, id FROM answer WHERE id = :p0';
        try {
    ...
    public function filterByLabel($label = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(AnswerTableMap::COL_LABEL, $label, $comparison);
    }

    /**
     * Filter the query on the id_question column
     *
     * Example usage:
     * <code>
     * $query->filterByIdQuestion(1234); // WHERE id_question = 1234
     * $query->filterByIdQuestion(array(12, 34)); // WHERE id_question IN (12, 34)
     * $query->filterByIdQuestion(array('min' => 12)); // WHERE id_question > 12
     * </code>
     *
     * @see       filterByQuestion()
     *
     * @param     mixed $idQuestion The value to use as filter.
     ...
     * @return $this|ChildAnswerQuery The current query, for fluid interface
     */
    public function filterByIdQuestion($idQuestion = null, $comparison = null)
    {
        if (is_array($idQuestion)) {
            $useMinMax = false;
            if (isset($idQuestion['min'])) {
                $this->addUsingAlias(AnswerTableMap::COL_ID_QUESTION, $idQuestion['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($idQuestion['max'])) {
                $this->addUsingAlias(AnswerTableMap::COL_ID_QUESTION, $idQuestion['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AnswerTableMap::COL_ID_QUESTION, $idQuestion, $comparison);
    }
    ...
    /**
     * Filter the query by a related \Tests\Model\Question object
     *
     * @param \Tests\Model\Question|ObjectCollection $question The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($question instanceof \Tests\Model\Question) {
            return $this
                ->addUsingAlias(AnswerTableMap::COL_ID_QUESTION, $question->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(AnswerTableMap::COL_ID_QUESTION, $question->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByQuestion() only accepts arguments of type \Tests\Model\Question or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Tests\Model\QuestionQuery A secondary query class using the current class as primary query
     */
    public function useQuestionQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinQuestion($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Question', '\Tests\Model\QuestionQuery');
    }
