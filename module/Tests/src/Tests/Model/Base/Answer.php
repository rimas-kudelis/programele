 ...
 *
 *
* @package    propel.generator.Tests.src.Tests.Model.Base
*/
abstract class Answer implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Tests\\Model\\Map\\AnswerTableMap';
    ...
    /**
     * Initializes internal state of Tests\Model\Base\Answer object.
     */
     ...
     * @return $this|\Tests\Model\Answer The current object (for fluent API support)
     */
    public function setLabel($v)
    {
        ...
        if ($this->label !== $v) {
            $this->label = $v;
            $this->modifiedColumns[AnswerTableMap::COL_LABEL] = true;
        }
    ...
    public function setIdQuestion($v)
    {
        ...
        if ($this->id_question !== $v) {
            $this->id_question = $v;
            $this->modifiedColumns[AnswerTableMap::COL_ID_QUESTION] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AnswerTableMap::translateFieldName('Label', TableMap::TYPE_PHPNAME, $indexType)];
            $this->label = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AnswerTableMap::translateFieldName('IdQuestion', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id_question = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AnswerTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = AnswerTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Tests\\Model\\Answer'), 0, $e);
        }
    }
    ...
    {
        if ($this->aQuestion !== null && $this->id_question !== $this->aQuestion->getId()) {
            $this->aQuestion = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AnswerTableMap::COL_LABEL)) {
            $modifiedColumns[':p' . $index++]  = 'label';
        }
        if ($this->isColumnModified(AnswerTableMap::COL_ID_QUESTION)) {
            $modifiedColumns[':p' . $index++]  = 'id_question';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'label':
                        ...
                        break;
                    case 'id_question':
                        $stmt->bindValue($identifier, $this->id_question, PDO::PARAM_INT);
                        break;
                    case 'id':
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Tests\Model\Answer
     */
     ...
     *
     * @return $this|\Tests\Model\Answer The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AnswerTableMap::COL_LABEL)) {
            $criteria->add(AnswerTableMap::COL_LABEL, $this->label);
        }
        if ($this->isColumnModified(AnswerTableMap::COL_ID_QUESTION)) {
            $criteria->add(AnswerTableMap::COL_ID_QUESTION, $this->id_question);
        }
        if ($this->isColumnModified(AnswerTableMap::COL_ID)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Tests\Model\Answer (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setIdQuestion($this->getIdQuestion());
        if ($makeNew) {
            $copyObj->setNew(true);
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Tests\Model\Answer Clone of current object.
     * @throws PropelException
     */
    ...
    public function getQuestion(ConnectionInterface $con = null)
    {
        if ($this->aQuestion === null && ($this->id_question !== null)) {
            $this->aQuestion = ChildQuestionQuery::create()->findPk($this->id_question, $con);
