...
use Tests\Model\Category as ChildCategory;
use Tests\Model\CategoryQuery as ChildCategoryQuery;
...
abstract class Question implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Tests\\Model\\Map\\QuestionTableMap';
    ...
    /**
     * Initializes internal state of Tests\Model\Base\Question object.
     */
     ...
     * @return $this|\Tests\Model\Question The current object (for fluent API support)
     */
    ...
    public function setIdCategory($v)
    {
        ...
        if ($this->id_category !== $v) {
            $this->id_category = $v;
            $this->modifiedColumns[QuestionTableMap::COL_ID_CATEGORY] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : QuestionTableMap::translateFieldName('Label', TableMap::TYPE_PHPNAME, $indexType)];
            $this->label = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : QuestionTableMap::translateFieldName('IdCategory', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id_category = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : QuestionTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Tests\\Model\\Question'), 0, $e);
        }
    }
    ...
    {
        if ($this->aCategory !== null && $this->id_category !== $this->aCategory->getId()) {
            $this->aCategory = null;
        }
    } // ensureConsistency
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->answersScheduledForDeletion !== null) {
                if (!$this->answersScheduledForDeletion->isEmpty()) {
                    \Tests\Model\AnswerQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(QuestionTableMap::COL_ID_CATEGORY)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Tests\Model\Question
     */
     ...
     *
     * @return $this|\Tests\Model\Question The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(QuestionTableMap::COL_ID_CATEGORY)) {
            $criteria->add(QuestionTableMap::COL_ID_CATEGORY, $this->id_category);
        }
        if ($this->isColumnModified(QuestionTableMap::COL_ID)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Tests\Model\Question (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Tests\Model\Question Clone of current object.
     * @throws PropelException
     */
    ...
    public function getCategory(ConnectionInterface $con = null)
    {
        if ($this->aCategory === null && ($this->id_category !== null)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aCategory->addQuestions($this);
             */
        }
    ...
    public function initAnswers($overrideExisting = true)
    {
        ...
        $this->collAnswers = new ObjectCollection();
        $this->collAnswers->setModel('\Tests\Model\Answer');
    }
    ...
    {
        if (null !== $this->aCategory) {
            $this->aCategory->removeQuestion($this);
        }
        $this->label = null;
